...
int main(int argc, char* argv[]) {
    ...
    std::vector<int> ef_search_configs = {10, 20, 50};

    if (rank == 0) {
        std::cerr << "\n[System] Starting Automated Grid Search Evaluation (MPI Pure HNSW)...\n";
    }

    for (int t : thread_configs) {
        for (int ef_search : ef_search_configs) {
            run_mpi_evaluation(t, ef_search, &hnsw_searcher, test_query, test_gt,
