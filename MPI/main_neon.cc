...
int main(int argc, char* argv[]) {
    ...
    for (int probe : nprobe_configs) {
        run_mpi_evaluation(thread_count, probe, &hnsw_searcher, test_query, test_gt,
                           test_number, vecdim, test_gt_d, k, csv_file, 
                           "IVF_HNSW", out_dir, local_base_number, rank, size, hnsw_m, hnsw_ef_construction);
    }

    if (rank == 0) {
        csv_file.close();
        std::ofstream meta(out_dir + "/run_info.txt");
        meta << "algorithm=IVF_HNSW_NEON\n";
