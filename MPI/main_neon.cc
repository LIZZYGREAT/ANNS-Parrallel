...
void run_mpi_evaluation(int thread_count, int ef_search, SearcherType* searcher,
    ...
    tp::parallel_region([&](int tid) {
        ...
        for (size_t i = start_idx; i < end_idx; ++i) {
            ...
            std::priority_queue<Candidate> pq_res = searcher->search(query_vec, k, ef_search);
    ...
    if (rank == 0) {
        ...
        std::cerr << "[Result] Method: " << method_name << " | Threads: " << thread_count
                  << " | ef_search: " << ef_search << " | Recall: " << final_recall
...
int main(int argc, char* argv[]) {
    ...
    // 初始化并构建纯 HNSW 索引
    HNSWIndex index(vecdim, hnsw_m, hnsw_ef_construction);
    index.build(local_base, local_base_number, rank, size);

    HNSWSearcher hnsw_searcher(&index);

    std::ofstream csv_file;
    if (rank == 0) {
        csv_file.open(out_dir + "/results.csv");
        csv_file << "Algorithm,Threads,ef_search,HNSW_M,HNSW_ef_c,Recall,MaxLocalLatency_us,AvgLocalLatency_us,ImbalanceRatio,QPS,MPIOverhead_us\n";
    }

    std::vector<int> ef_search_configs = {10, 20, 40, 80, 160};

    if (rank == 0) {
        std::cerr << "\n[System] Starting Fixed-Thread Single-Loop Evaluation (Pure HNSW NEON)...\n";
    }

    for (int ef_search : ef_search_configs) {
        run_mpi_evaluation(thread_count, ef_search, &hnsw_searcher, test_query, test_gt,
                           test_number, vecdim, test_gt_d, k, csv_file, 
                           "PURE_HNSW", out_dir, local_base_number, rank, size, hnsw_m, hnsw_ef_construction);
    }

    if (rank == 0) {
        csv_file.close();
        std::ofstream meta(out_dir + "/run_info.txt");
        meta << "algorithm=PURE_HNSW_NEON\n";
        meta << "mpi_size=" << size << "\n";
        meta << "local_base_per_node=" << local_base_number << "\n";
        meta << "hnsw_m=" << hnsw_m << "\n";
        meta << "hnsw_ef_construction=" << hnsw_ef_construction << "\n";
        meta.close();
        std::cerr << "\n[System] Server-side evaluation pipeline completed successfully.\n";
